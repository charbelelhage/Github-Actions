import os
from fastapi import BackgroundTasks, FastAPI, Request, File, UploadFile,Form
...
@app.post("/upload")
async def upload_file(bagfile: UploadFile = File(...), start_time: float = Form(...), end_time: float = Form(...),
                      output_folder: str = Form(...), topics_dropdown: str = Form(...)):
    ...
    if output_folder is None:
        output_folder = "/" #Checking if the output folder value was null, assign it the value "/" to save the images in the same directory

    last_character=output_folder[-1] #to get the last character of the output folder specified
    if(last_character!="/"):
        output_folder=output_folder + "/" #to add / to the end of the output folder to be able to create the directory

    filename = bagfile.filename
    extension = os.path.splitext(filename)[1] #Split the filename to get the file extention
    if extension != ".bag": #if the user has uploaded a file different than a bag file, the api will return a response with status fail and a message.
        data = {
            "status": "fail",
    ...
    if(topics_dropdown =="/rgb_image"):
        #Command that will call the python script responsible of extracting the images depending on the arguments sent by the form.
        ...
        #This call is used to run the command in the background so the function won't wait for the command to finish execution to return a response.
        ...
        #Command that will call the python script responsible of extracting the pointclouds depending on the arguments sent by the form.
