...
async def test_upload_file_with_valid_bag(tmp_path):  # Using tmp_path fixture
    ...
    with open(bagfile_path, "wb") as f:
        f.write(bagfile_content)
    ...
    response = await upload_file(bagfile=bagfile, start_time=start_time, end_time=end_time, output_folder=output_folder, topics_dropdown=topics_dropdown)
...
async def test_upload_file_with_invalid_bag(tmp_path):  # Using tmp_path fixture
    ...
    # Call the upload_file function within the event loop, passing the UploadFile object
    response = await upload_file(bagfile=invalid_file, start_time=start_time, end_time=end_time, output_folder=output_folder, topics_dropdown=topics_dropdown)
